...
import { vertexShader } from './figure-motion';
import { createFigureShadow } from './figure-shadow';
...
export function FigureMesh({ shape, motion, onReady, neuromancer, signalMotion, environmentRef, shadowRef, staffShape }: {
  shape: string;
  motion: boolean;
  ...
  shadowRef: RefObject<HTMLCanvasElement | null>;
  staffShape: string;
}) {
  ...
  useEffect(() => {
    ...
    const cleanups: (() => void)[] = [];
    onReady(false);
    const shadow = shadowRef.current ? createFigureShadow(shadowRef.current, shape, staffShape) : null;
    if (shadow) cleanups.push(shadow.dispose);
    ...
    async function initialize() {
      ...
      draw = () => {
        shadow?.render(elapsed, motionState.current);
  ...
  }, [shape, staffShape, onReady, contextVersion, environmentRef, shadowRef]);
